...
load_dotenv('versions/v1/config.env')  # Load version-specific config
...
def deploy_contract(joueur_a_private_key, secret_number, max_attempts):
...
def jouer_partie(joueur_b_private_key, contrat_address):
    ...
    while tentatives_restantes > 0:
        try:
            ...
            tx_signe = compte.sign_transaction(tx)
            tx_hash = w3.eth.send_raw_transaction(tx_signe.raw_transaction)
            # Adjusted timeout for waiting for transaction receipt
            receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=30)
