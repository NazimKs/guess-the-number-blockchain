...
load_dotenv('versions/v4/config.env')  # Load version-specific config
...
def hash_number(number):
    """Hash a number using keccak256"""
    return Web3.keccak(text=str(number))
...
def deploy_contract(joueur_a_private_key, secret_number, max_attempts, stake=None):
    ...
    secret_hash = hash_number(secret_number)
    ...
    tx = contrat.constructor(secret_hash, max_attempts, has_staking).build_transaction(tx_params)
...
def jouer_partie(joueur_b_private_key, contrat_address):
    ...
    while tentatives_restantes > 0:
        try:
            ...
            # Hash the guess
            proposition_hash = hash_number(proposition)
            
            # Prepare the transaction
            tx = contrat.functions.deviner(proposition_hash).build_transaction({
                'from': compte.address,
                'nonce': w3.eth.get_transaction_count(compte.address)
            })
